...
import au.web.odata.entityProcessor.processor.EntityProcessorException;
...
public class GenericEntityProcessor implements EntityCollectionProcessor, EntityProcessor, PrimitiveProcessor, PrimitiveValueProcessor, ComplexProcessor {
	...
	// This constructor is application specific and not mandatory for the Olingo
	// library. We use it here to simulate the
	// database access
	public GenericEntityProcessor() {
	...
	public void readEntityCollection(final ODataRequest request, ODataResponse response, final UriInfo uriInfo, final ContentType requestedContentType) throws ODataApplicationException, SerializerException {
		...
		InputStream serializedContent = serializer.entityCollection(edm, edmEntitySet.getEntityType(), entitySet, EntityCollectionSerializerOptions.with().id(id).contextURL(isODataMetadataNone(requestedContentType) ? null : getContextUrl(edmEntitySet, false, expand, select, null)).count(uriInfo.getCountOption()).expand(expand).select(select).build()).getContent();
	...
	public void readEntity(final ODataRequest request, ODataResponse response, final UriInfo uriInfo, final ContentType requestedContentType) throws ODataApplicationException, SerializerException {
		...
		if (entity == null) {
			...
			final ExpandOption expand = uriInfo.getExpandOption();
			final SelectOption select = uriInfo.getSelectOption();
			InputStream serializedContent = serializer.entity(edm, edmEntitySet.getEntityType(), entity, EntitySerializerOptions.with().contextURL(isODataMetadataNone(requestedContentType) ? null : getContextUrl(edmEntitySet, true, expand, select, null)).expand(expand).select(select).build()).getContent();
			response.setContent(serializedContent);
			response.setStatusCode(HttpStatusCode.OK.getStatusCode());
			response.setHeader(HttpHeader.CONTENT_TYPE, requestedContentType.toContentTypeString());
		}
	...
	private Entity readEntityInternal(final UriInfoResource uriInfo, final EdmEntitySet edmEntitySet) throws EntityProcessorException {
		...
		for (String entityName : entityProcessors.keySet()) {
			...
			if (entityProcessor.getEntitySetName().equals(edmEntitySet.getName())) {
				result = entityProcessor.read(edmEntitySet, resourceEntitySet.getKeyPredicates());
				break;
			}
		}
	...
	@Override
	public void updateEntity(final ODataRequest request, final ODataResponse response, final UriInfo uriInfo, final ContentType requestFormat, final ContentType responseFormat) throws ODataApplicationException, DeserializerException, SerializerException {
